...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['accept', 'decline']) && $aid) {
        try {
            ...
            $newStatus = $action === 'accept' ? 'in_progress' : 'declined';
            $db->prepare("UPDATE review_assignments SET assignment_status = :st WHERE id = :aid AND reviewer_id = :uid")
...
$statuses = [
    ...
    'in_progress' => ['label_th' => 'กำลังประเมิน',  'label_en' => 'In Progress', 'color' => '#0057b7'],
    'completed'   => ['label_th' => 'เสร็จสิ้น',      'label_en' => 'Completed',   'color' => '#198754'],
    'declined'    => ['label_th' => 'ปฏิเสธ',         'label_en' => 'Declined',    'color' => '#bd3838'],
];
?>
<!DOCTYPE html>
<html lang="<?= $_lang ?>">
    ...
    <?php else: ?>
      <div class="d-flex flex-column gap-3">
        <?php foreach ($assignments as $ra):
          ...
          $sInfo      = $statuses[$ra['assignment_status']] ?? ['label_th'=>$ra['assignment_status'],'label_en'=>$ra['assignment_status'],'color'=>'#6c757d'];
        ?>
              ...
              <?php elseif ($ra['assignment_status'] === 'completed'): ?>
                ...
                <?php if ($ra['score_overall']): ?>
                  <span style="font-size:.85rem;font-weight:700;color:var(--blue-dark);">
                    <?= number_format($ra['score_overall'],1) ?>/10
                  </span>
                <?php endif; ?>
              <?php endif; ?>
            </div>
